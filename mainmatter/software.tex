...
\software{v-scenario} provides a simple top-view rendering system, that displays the scenario and the entities \software{v-traffic} is handling.

\subsection{Coordinate System} % TODO: check

\begin{figure}[!ht]
	\FLOATcenter{}
	\FIGUREinclude[width=\FLOATtextwidth]{mainmatter/images/software/vtdcoords.pdf}
...
The absolute coordinate system is an Euclidean space on three axes, with the $(0,0)$ origin being the center point of the scenario. Measures are in meters (\si{\meter}).
...
The reference point and $(0,0)$ origin for the geometry of each vehicle is the mid point of the rear wheels axle. Measures are also in meters (\si{\meter}).
...
Rotations are exposed as Euclidean angles. Measures are in degrees \si{\degree}.
...
\begin{itemize}
	...
	\item \code{yaw}: insists on the \code{x}-\code{y} plane; unwraps from the positive \code{x} semi-axis; $\ang{0} \leqslant \alpha \leqslant \ang{360}$ clockwise
	\item \code{roll}: insists on the \code{y}-\code{z} plane; unwraps from the positive \code{y} semi-axis; $\ang{-180} \leqslant \alpha \leqslant \ang{0}$ in the negative (lower) semiplane, $\ang{0} \leqslant \alpha \leqslant \ang{180}$ in the positive (upper) semiplane\footnote{The roll angle visualization is left to the reader.}
\end{itemize}
